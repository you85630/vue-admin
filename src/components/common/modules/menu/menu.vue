<template>
  <ul class="menu-box">
    <li class="menu-li" :class="{'open-menu':item.expand}" v-for="(item, index) in data" :key="'item'+index" @click="selectMenu(item)">
      <div class="title-box" :class="{'menu-active':item.select,'title-box-open':item.expand}" @click="openMenu(item,item.link)">
      ...
      <div class="menu-main" v-show="item.expand">
        <menu-li :class="'menu-li-'+index" :data="item.children" v-if="item.children" @on-change="selectMenu"></menu-li>
      </div>
    </li>
  </ul>
</template>
